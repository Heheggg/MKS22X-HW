...
public class Maze{
    ...
    */
	...
	try{
	    ...
	    maze = new char[save.size()][save.get(0).length()];
	    for(int i = 0; i < save.size(); i++){
		for(int i2= 0; i2 < save.get(i).length(); i2++){
		    maze[i][i2] = save.get(i).charAt(i2);
		    if(maze[i][i2] == 'S'){
			existS = true;
			startx = i;
			starty = i2;
	    ...
	    if(!existS){
		throw new IllegalArgumentException("We don't have a start");
	    }
